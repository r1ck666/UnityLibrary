Shader "Custom/CurveRoadShader_HighQuality"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                // BazierLine
                float t = (float)v.vertex.z / _Length;

                // Bazier dt
                float4 bSloap = -3.0f * (t - 1.0f) * (t - 1.0f) * _P0 +
                3.0f * (3.0f * t -1.0f) * (t - 1.0f) * _P1 +
                -3.0f * (3.0f * t - 2.0f) * t * _P2 +
                3.0f * t * t * _P3;

                float3 c = normalize (cross(bSloap, float3(0.0f, 1.0f, 0.0f)));
                if (c.x < 0) c = c * -1.0f;
                ...
                float4 b = oneMinusT * oneMinusT * oneMinusT * _P0 + 
                    ...
                    t * t * t * _P3 +
                    float4(v.vertex.x * c.x, 0, v.vertex.x * c.z, 0);
